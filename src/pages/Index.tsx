...
const Index = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      } = await supabase.from('survey_responses').select('*').order('created_at', {
        ascending: false
      });
      if (responsesError) {
      ...
      const {
        data: metadataData,
        error: metadataError
      } = await supabase.from('survey_metadata').select('*').order('last_updated', {
        ascending: false
      }).limit(1).maybeSingle();
  ...
  const happyPercentage = processedData ? (processedData.happinessDistribution.filter(h => h.nivel >= 4).reduce((sum, h) => sum + h.count, 0) / totalResponses * 100).toFixed(1) : "0.0";
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Dashboard SIPAL - Análise
              </p>
            </div>
            <img src={logoSipal} alt="Logo SIPAL" className="h-16 w-auto object-contain" />
          </div>
        </div>
      </header>

      <main className="container mx-auto px-4 py-8 pb-16">
        {responses.length === 0 ? <div className="max-w-2xl mx-auto">
            <FileUpload onDataLoaded={handleDataLoaded} />
          </div> : <div className="space-y-8">
            {/* Filters */}
            <div className="max-w-xs">
              <FilterPanel selectedUnit={selectedUnit} onUnitChange={setSelectedUnit} />
            </div>

            {/* KPI Cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <MetricCard title="Média Geral de Felicidade" value={processedData?.averageHappiness.toFixed(1) || "0.0"} icon={Heart} subtitle="Escala de 1 a 5" />
              <MetricCard title="Colaboradores Felizes" value={`${happyPercentage}%`} icon={TrendingUp} subtitle="Notas 4 e 5" />
            ...
            {showComments && <div className="bg-card border border-border rounded-lg p-6 shadow-sm">
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto">
                  {filteredResponses.filter(r => r.comentarios && r.comentarios.trim() !== "").map((response, index) => <div key={index} className="bg-muted/30 rounded-lg p-4 border border-border/50">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-card/80 backdrop-blur-sm border-t border-border/50 py-2">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between gap-4">
            ...
            {responses.length > 0 && <FileUpload onDataLoaded={handleDataLoaded} compact />}
          </div>
        </div>
      </footer>
    </div>;
