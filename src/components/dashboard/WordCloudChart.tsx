...
export function WordCloudChart({
  ...
  const words = useMemo(() => {
    ...
    const criticalTerms = new Set(['lideranca', 'lider', 'gestao', 'gestor', 'reconhecimento', 'valorizacao', 'comunicacao', 'sobrecarga', 'carga', 'pressao', 'salario', 'remuneracao', 'desenvolvimento', 'oportunidade', 'crescimento', 'equilibrio', 'flexibilidade']);
  ...
  return <Card className="shadow-sm" ref={chartRef}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-foreground">
            Principais temas mencionados nas respostas abertas
          </CardTitle>
      ...
      <CardContent>
        <div className="flex flex-wrap gap-3 justify-center items-center min-h-[400px] p-6">
          {words.map((word, index) => <span key={index} style={{
        ...
        }} className="cursor-pointer hover:opacity-100 hover:scale-110" title={`${word.text}: ${word.value} menções${word.isCritical ? ' (termo prioritário)' : ''}`}>
              {word.text}
            </span>)}
        </div>
      </CardContent>
