...
interface UnhappinessFactorsChartProps {
...
export function UnhappinessFactorsChart({ data }: UnhappinessFactorsChartProps) {
  ...
  const exportChart = async (format: 'png' | 'pdf') => {
    ...
    if (format === 'png') {
      ...
      link.download = 'motivos-infelicidade.png';
      ...
      pdf.save('motivos-infelicidade.pdf');
    }
  };
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-foreground flex items-center gap-2">
            ...
            Top 5 fatores menos felicidade
          </CardTitle>
          ...
          <BarChart data={chartData} margin={{ left: 20, right: 20, bottom: 80 }}>
            ...
            >
              {chartData.map((_, index) => (
                <Cell key={`cell-${index}`} fill="hsl(var(--chart-danger))" />
              ))}
              <LabelList 
